		...
		>
			...
			{!isXlScreen && (
				<div
					className={`w-full h-64 xl:hidden ${
						isXlScreen ? 'hidden' : 'flex items-center justify-center'
					}`}
				>
			...
			<div
				className={`flex flex-col h-full xl:text-left text-center xl:items-start xl:justify-center justify-start items-center ${
					isXlScreen
						? 'w-full sm:w-auto sm:px-28 sm:mt-[-17rem] sm:mr-[29rem]'
						: 'w-full p-4'
				}`}
			>
				<motion.div
					className='mb-10 mt-4 text-2xl font-medium !leading-[1.5] sm:text-4xl w-full text-center'
				...
				>
					...
					<span className='xl:w-2/3 w-full font-bold gradient-text'>
						Full-stack web developer
					</span>

					<div className='text-xl mt-2'>
						<span>6+ years building enterprise-level</span>{' '}
				...
				>
					<Link
						className='group bg-white px-7 py-3 flex items-center gap-2 rounded-full outline-none focus:scale-110 hover:scale-110 active:scale-105 transition cursor-pointer borderBlack dark:bg-white/10 mr-4'
			...
			{/* Avatar */}
			{isXlScreen && (
				<div className='hidden xl:block absolute inset-y-0 right-0 w-full h-full xl:w-[50%]'>
